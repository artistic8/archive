...
foreach ($dir as $fileinfo) {
    if (!$fileinfo->isDot() && $fileinfo->isDir() && preg_match("/^[0-9]+$/", $fileinfo->getFilename())) {
        ...
        foreach($bets as $raceNumber => $data){
            ...
            $expr =  count($winInter) . array_sum($winInter) . count($qinInter) . array_sum($qinInter) . count($winPivots) . array_sum($winPivots);
...
$outtext .= '$favoritePlace = ' . "['" . implode("', '", $favoritesPlaceKeys) . "'];\n";
