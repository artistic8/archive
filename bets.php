...
for ($raceNumber = 1; $raceNumber <= $totalRaces; $raceNumber++) {
    ...
    if(count($inter) >= 2 && count($favorites) >= 3){
        ...
        $total += $winwonAmount;
        $winbetAmount = 100 * count($set3);
        if(!empty(array_intersect($set3, array_slice($officialWin, 0, 1)))) $winwonAmount = 10 * $winAmount - $winbetAmount;
        else $winwonAmount = 0 - $winbetAmount;
