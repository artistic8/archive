...
for ($raceNumber = 1; $raceNumber <= $numberOfRaces; $raceNumber++) {
    ...
    if(count($runners) >= 10 && count($winCheck) > 1 && count($trioCheck) > 1 && count($qinInter) >= 2 && count($favorites) >= 3 && count($favorites) < 6){
    ...
    if(isset($officialWin) && $totalBets[$raceNumber] > 0){
        ...
        //     }
        // }
        if(count($runners) >= 10 && count($winCheck) > 1 && count($trioCheck) > 1 && (count($winInter) >= 2 || count($qinInter) >= 2 || count($trioInter) >= 2) && count($favorites) >= 3 && count($favorites) < 6){
