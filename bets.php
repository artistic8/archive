...
for ($raceNumber = 1; $raceNumber <= $totalRaces; $raceNumber++) {
    ...
    if(count($inter) >= 2 && count($favorites) >= 3){
        ...
        $racetext .= "\t\t'win($20)' => '" . implode(", ", $set3) . "',\n"; 
        $racetext .= "\t\t'qin/trio($10)' => '" . implode(", ", $favorites) . "',\n"; 
        $unitBet = 1;
        $racetext .= "\t\t'qin($10)' => '" . implode(", ", $favorites) . " X " . implode(", ", $set2)  . "',\n"; 
        $winbetAmount = 10 * $unitBet * count($favorites);
        if(!empty(array_intersect($favorites, array_slice($officialWin, 0, 1)))) $winwonAmount = $unitBet * $winAmount - $winbetAmount;
        else $winwonAmount = 0 - $winbetAmount;
        $racetext .= "\t\t'win(favorites)' => $winwonAmount,\n";
        $totalWin += $winwonAmount;
        $total += $winwonAmount;
        $winbetAmount = 20 * $unitBet * count($set3);
        if(!empty(array_intersect($set3, array_slice($officialWin, 0, 1)))) $winwonAmount = 2 * $unitBet *$winAmount - $winbetAmount;
        else $winwonAmount = 0 - $winbetAmount;
