...
for ($raceNumber = 1; $raceNumber <= $numberOfRaces; $raceNumber++) {
    ...
    $unitBet = 100;
    $winCondition = (count($qininter) >= 2 || (count($qininter) > 1 && count($winpivots) === 2 && in_array(end($favorites), $winpivots))) 
    && 
    (count($pivots) === 3);
    $placeCondition = (count($wininter) >= 2 || (count($wininter) > 1 && count($winpivots) === 2 && in_array(end($favorites), $winpivots))) 
    && 
    (count($pivots) === 3);
    if(
        $placeCondition
