...
for ($raceNumber = 1; $raceNumber <= $numberOfRaces; $raceNumber++) {
    ...
    $unitBet = 100;
    $winCondition = count($qininter) !== 1 && in_array(end($favorites), $winpivots) && count($pivots) === 3;
    $placeCondition =count($wininter) !== 1 && in_array(end($favorites), $winpivots) && count($pivots) === 3;
    if(
        $placeCondition
    ){
