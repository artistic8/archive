...
for ($raceNumber = 1; $raceNumber <= $numberOfRaces; $raceNumber++) {
    ...
    $unitBet = 100;
    $winCondition = (count($qininter) >= 2 || (count($qininter) > 0 && count($winpivots) === 2 && in_array(end($favorites), $winpivots))) 
    && 
    (count($pivots) === 3);
    $placeCondition = (count($wininter) >= 2 || (count($wininter) > 0 && count($winpivots) === 2 && in_array(end($favorites), $winpivots))) 
    && 
    (count($pivots) === 3);
    if(
        $placeCondition
