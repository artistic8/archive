...
for ($raceNumber = 1; $raceNumber <= $totalRaces; $raceNumber++) {
    ...
    if(count($inter) >= 2 && count($favorites) >= 3){
        ...
        $winbetAmount = 51 * count($favorites);
        if(!empty(array_intersect($favorites, array_slice($officialWin, 0, 1)))) $winwonAmount = 1 * $winAmount - $winbetAmount;
        else $winwonAmount = 0 - $winbetAmount;
        $racetext .= "\t\t'win(favorites)' => $winwonAmount,\n";
        $totalWin += $winwonAmount;
        $total += $winwonAmount;
        $winbetAmount = 20 * count($set3);
        if(!empty(array_intersect($set3, array_slice($officialWin, 0, 1)))) $winwonAmount = 2 * $winAmount - $winbetAmount;
        else $winwonAmount = 0 - $winbetAmount;
