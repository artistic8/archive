...
for ($raceNumber = 1; $raceNumber <= $numberOfRaces; $raceNumber++) {
    ...
    if(count($surePlace) > 1 && count($favorites) >= 3 && !empty(array_intersect($surePlace, $favorites)) && count(array_intersect($surePlace, $favorites)) < 2){
    ...
    if(isset($officialWin) && $totalBets[$raceNumber] > 0 && count($surePlace) > 1 && count($favorites) >= 3 && !empty(array_intersect($surePlace, $favorites)) && count(array_intersect($surePlace, $favorites)) < 2){
        ...
        if(count($fabor) === 1  && in_array($fabor[0], array_slice($officialWin, 0, 3)) && isset($placeAmount[$fabor[0]])){
            ...
            $totalPlace += 100 * $placeAmount[$fabor[0]];
        }
