<?php

return [
    'all races' => [
        'Ok numbers' => [7],
        'Nok numbers' => [1, 2, 3, 4, 5, 6, 8, 9, 10, 11, 12],
        'Ok jockeys' => ['B Avdulla', 'A Badel', 'Y L Chung', 'J McDonald', 'E C W Wong', 'M L Yeung', 'C L Chau'],
        'Nok jockeys' => ['A Atzeni', 'H Bentley', 'H Bowman', 'M Chadwick', 'L Ferraris', 'L Hewitson', 'C Y Ho', 
            'K C Leung', 'K De Melo', 'K Teetan', 'M F Poon', 'Z Purton'],
    ],
    1 => [
        'Ok numbers' => [2, 5, 7],
        'Nok numbers' => [3],
        'Ok jockeys' => ['K Teetan', 'Y L Chung', 'Z Purton'],
        'Nok jockeys' => ['A Atzeni', 'H Bowman'],
    ],
    2 => [
        'Ok numbers' => [5, 7, 10],
        'Nok numbers' => [1, 8, 9],
        'Ok jockeys' => ['B Avdulla', 'K Teetan'],
        'Nok jockeys' => ['H Bowman', 'M Chadwick', 'Z Purton'],
    ],
    3 => [
        'Ok numbers' => [3],
        'Nok numbers' => [1, 4, 9, 11],
        'Ok jockeys' => ['K Teetan', 'Z Purton', 'A Badel'],
        'Nok jockeys' => ['H Bowman', 'C Y Ho', 'H Bentley', 'M F Poon'],
    ],
    4 => [
        'Ok numbers' => [1, 5, 7],
        'Nok numbers' => [3, 6, 10],
        'Ok jockeys' => ['C Y Ho', 'E C W Wong', 'Y L Chung'],
        'Nok jockeys' => ['H Bowman', 'L Hewitson', 'K Teetan'],
    ],
    5 => [
        'Ok numbers' => [7, 8],
        'Nok numbers' => [4, 5],
        'Ok jockeys' => ['K Teetan'],
        'Nok jockeys' => ['K De Melo', 'Z Purton', 'L Hewitson'],
    ],
    6 => [
        'Ok numbers' => [7, 10],
        'Nok numbers' => [3, 6, 12],
        'Ok jockeys' => ['A Badel', 'H Bowman', 'K De Melo', 'C Y Ho'],
        'Nok jockeys' => ['Z Purton', 'L Ferraris', 'A Hamelin'],
    ],
    7 => [
        'Ok numbers' => [6, 8, 11],
        'Nok numbers' => [2, 3],
        'Ok jockeys' => ['A Atzeni', 'A Badel', 'K Teetan', 'J McDonald', 'C L Chau'],
        'Nok jockeys' => ['C Y Ho'],
    ],
    8 => [
        'Ok numbers' => [5, 6, 11],
        'Nok numbers' => [2, 3, 8],
        'Ok jockeys' => ['B Avdulla', 'L Hewitson'],
        'Nok jockeys' => ['A Atzeni', 'H Bowman', 'K Teetan'],
    ],
    9 => [
        'Ok numbers' => [11],
        'Nok numbers' => [],
        'Ok jockeys' => ['M L Yeung'],
        'Nok jockeys' => [],
    ],
    10 => [
        'Ok numbers' => [11],
        'Nok numbers' => [4],
        'Ok jockeys' => ['C L Chau'],
        'Nok jockeys' => ['K C Leung'],
    ],
    11 => [
        'Ok numbers' => [],
        'Nok numbers' => [],
        'Ok jockeys' => [],
        'Nok jockeys' => [],
    ],
];



?>