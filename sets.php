...
for ($raceNumber = 1; $raceNumber <= $numberOfRaces; $raceNumber++) {
    ...
    if(!empty($allValues) && count($allValues) <= 7) $values1 = $allValues;
    else $values1 = [];
    $copy = $runners;
    sort($copy);
    foreach($runners as $one){
        ...
        $allValues = getAllValues($runners, $raceNumber);
        if(!empty($allValues) && count($allValues) <= 7) $values1 = array_values(array_unique(array_merge($values1, $allValues)));
        foreach($runners as $two){
    ...
    $runners = $copy;
    $allValues = getAllValues($runners, $raceNumber);
    if(!empty($allValues) && count($allValues) <= 7) $values2 = $allValues;
    else $values2 = [];
    foreach($runners as $one){
        $runners = array_diff($runners, [$one]);
        $allValues = getAllValues($runners, $raceNumber);
        if(!empty($allValues) && count($allValues) <= 7) $values2 = array_values(array_unique(array_merge($values2, $allValues)));
        foreach($runners as $two){
            $runners = array_diff($runners, [$two]);
            $allValues = getAllValues($runners, $raceNumber);
            if(!empty($allValues) && count($allValues) <= 7) $values2 = array_values(array_unique(array_merge($values2, $allValues)));
